...
import qualified Pos.Block.Logic as DB (getHeadersFromManyTo, getHeadersFromToIncl)
...
logicLayerFull jsonLogTx k =
    bracket acquire release $ \_ -> do
        ...
        let
            ...
            getBlockHeaders' older newer = fmap Right (DB.getHeadersFromToIncl DB.getHeader older newer)
